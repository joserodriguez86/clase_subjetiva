...
pacman::p_load(tidyverse, haven, gridExtra, grid, occupar, ggpubr, ggsci, gtsummary,
               survey, jtools, huxtable, nnet, broom, kableExtra, flextable)
...
wvs <- wvs %>%
  mutate(clase_sub = car::recode(X045, "1:2 = 1; 3=2; 4=3; 5=4; else=NA"),
         clase_sub_f = factor(clase_sub, labels = c("Media-alta",
...
base2021 <- base2021 %>%
  mutate(egp = case_when(
    ...
    categoria == 3 & ciuo %in% c(5411, 5412) ~ 10, #REVISARRRRRRRRRRRRRRRRRRRRRRRRRR
...
#Países 2017-2022
wvs %>%
  filter(S003 %in% c(32, 68, 76, 152, 170, 218), S002 == 7) %>%
  drop_na(clase_sub_f) %>%
...
ggsave(filename = "salidas_articulo/barras_wvs_paises.jpg", dpi = 300, width = 8, height = 6)
