...
base2021 %>%
  ...
  tbl_svysummary(include = c(egp5_f, egp5_origen_f, nivel_ed_f, sexo_f, edad_grupo,
                             formal, afiliacion, rama, politica),
...
base2021 <- base2021 %>% 
  mutate(clasesub4 = factor(car::recode(M11.6, "1:3 = 1; 4=2; 5=3; 6=4"),
                            labels = c("Clase media", "Clase media-baja", "Clase trabajadora",
                                       "Clase baja")),
         clasesub4 = relevel(clasesub4, ref = "Clase baja"))

multinomial <- multinom(clasesub4 ~ egp5_f + sexo_f + edad_grupo + nivel_ed_f + formal + rama +
                   afiliacion + politica + egp5_origen_f, data = base2021, weights = POND2R_FIN_n,
                   trace = F)

tidied <- tidy(multinomial)
models <- list()
models[["Media vs baja"]] <- tidy_replace(multinomial, tidied[tidied$y.level == "Clase media", ])
models[["Media-baja vs baja"]] <- tidy_replace(multinomial, tidied[tidied$y.level == "Clase media-baja", ])
models[["Trabajadora vs baja"]] <- tidy_replace(multinomial, tidied[tidied$y.level == "Clase trabajadora", ])

reg_multinomial <- export_summs(models, statistics = c("AIC", "deviance"), exp = T,
             stars = c(`***` = 0.01, `**` = 0.05, `*` = 0.1))
